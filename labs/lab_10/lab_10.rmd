...
**A1.** https://datawrapper.dwcdn.net/vMzi3/1/

```{r}
md_gov_county <- read_csv("data/md_gov_county.csv") %>% 
  clean_names() %>%
  rename(county_22 = county) %>% 
  rename(write_ins_22 = write_ins) %>% 
  mutate(total_22 = rowSums(md_gov_county[, 3:9])) %>% 
  mutate(cox_percent_22 = (cox/total_22)*100)

md_gov_county_18 <- read_csv("data/md_gov_county_18.csv") %>% 
  clean_names() %>%
  rename(county_18 = county) %>% 
  rename(write_ins_18 = write_ins) %>%
  mutate(total_18 = rowSums(md_gov_county[, 3:7])) %>%
  mutate(hogan_percent_18 = (hogan/total_18)*100)

combined_md_gov_results <- md_gov_county %>% left_join(md_gov_county_18, by="geoid")

combined_md_gov_results <- combined_md_gov_results %>% 
  mutate(cox_hogan_diff = cox_percent_22 - hogan_percent_18)

 write_csv(combined_md_gov_results, "data/combined_md_gov_results.csv")
...
**A2.** https://datawrapper.dwcdn.net/5UGmZ/1/ The story here is the decline in voters by mail from the pandemic year in 2020 to 2022. Only 7.1 percent of voters voted by mail in 2018, while in 2020 49.5% of voters voted by mail. This decreases in 2022 when 19.3% of voters voted by mail. 
...
percent_md_votes_by_type <- md_votes_by_type %>% 
  select(year, pct_early, pct_election_day, pct_mail)
  

write_csv(percent_md_votes_by_type, "data/percent_md_votes_by_type.csv")
```
...
**A3.** https://datawrapper.dwcdn.net/kvXt2/1/ The story here is Montgomery County zip codes comprising majority of Act Blue donors in Maryland. Why isn't other wealthy counties such as Howard County donating more to Act Blue or Prince George's County, one of the wealthiest Black counties in the states.
...
md_actblue <- md_actblue %>% 
  mutate(pct_total = (total/20328513)*100)
write_csv(md_actblue, "data/md_actblue_zip.csv")
```
