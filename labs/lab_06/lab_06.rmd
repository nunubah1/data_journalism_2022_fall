...
```{r}
# Party registrations
new_party_registrations <- read_csv("data/tabula-new_party_registrations.csv") %>% clean_names()

new_party_registrations <- new_party_registrations %>% 
  rename(types_of_registrations = x1) %>% 
  select(-dups)
new_party_registrations
glimpse(new_party_registrations)
```

```{r}
# Removals
party_removals <- read_csv("data/tabula-party_removals.csv") %>% clean_names()
...
```{r}
# Changes
registration_changes <- read_csv("data/tabula-registration_changes.csv") %>% clean_names()

registration_changes <- registration_changes%>% 
  rename(counties = x1)

registration_changes
glimpse(registration_changes)
  
```

```{r}
# Current registrations
current_county_registrations <- read_csv("data/tabula-current_county_registrations.csv") %>% clean_names()

current_county_registrations <- current_county_registrations %>% 
