...
county_most_voting_locations <- moore_miller_votes %>% 
  select(county_name, precinct_code, location_name, location_address, city)
```

```{r}
county_most_voting_locations <- county_most_voting_locations%>% 
  group_by(county_name) %>% 
  summarize(count = n()) %>% 
  arrange(desc(count))

county_most_voting_locations
````
...
governor_total_votes <- governor_total_votes %>% 
  ...
  select(county_name, total_votes)
...
```{r}
mean_median_votes <- total_votes_primary %>% 
  ungroup() %>% 
  summarize(
            votes = votes
            mean_votes = mean(votes)
            median_votes = median(votes)) %>% 
  select(county_name, mean_votes, median_votes) %>% 
  arrange(desc(votes))
```
