...
**Answer** The code added a new column calculating the percentage of votes being cast on election day. It calculates the percentage by dividing the election day votes column by the votes (all votes) column. It gives us a percentage decimal (ex:.7845036) It does not make the column permanent unless we save it as a new variable. 
...
**Answer** This one makes the percent election day column easier to read by making it an actual percentage. IT multiplies by 100 the results of the election_day column divided by total votes column. It then organizes the result from most to least. 
...
**Answer** Yes because. We now see that Democrats are the ones that vote early or send absentee ballots. Like outside of the Talbot jursidiction on the first page, it's not until the 4th page before you see another Republican party candidate (in Talbot county of course) being voted for on election days, comparatively speaking, lower percentages.
...
**Answer** str_to_lower function which converts all the column to lower case and str_to_title which only capitalizes the first letter.
...
**Answer** It creates a new column that organizes the amount column in 4 categories: Less than $100, between $100 and $500, between $501 and $2900, more than $2,900. It allows for easier viewing or filtering of the statistics. Especially if the viewer is skimming or if the viewer wants to see the range of amounts donated. If the amount category only existed. It would be overwhelming to view.
...
**Answer** It groups all the votes into one category including the election day, absentee, provisional, and second absentee. It's important if you just want to see the total votes a candidate has and don't really want to see the breakdown. Not all stories want to tell the breakdown of votes. They just want to read the total votes. Like imagine if it was possible for CNN's election coverage to show the voter breakdown like the first chart showed. It would be overwhelming for viewers and they probably would tune out. So you have to keep your audience in mind. 
...
**Answer:** It changes the date formate that we use in the United States to the standard date format that is used worldwide.
...
**Answer** It's organizes the entire month values from most to least with June and July having the most total amount by a wide margin. The other chart just showed specific days but this shows the entire month even though it has the 1st of the month of every month shown. 
