...
**Answer** "One or more parsing issues, see 'problems()' for details Rows: 476915 Columns: 13- Column specific action
...
**Answer** 1640 problems are shown. The problem that R is describing is that since it has read the columns as a logical data type of true or false with only limited information, that when it ran into date or char strings, it didn't know what to do.
...
**Answer** It does not show any parsing errors this time around. 
...
**Task** Run the following codeblock to glimpse the data.  What data type is the "district" field?  What data type is the "precinct" field? What data type is the id column? Answer below.
**Answer** The district field is a double (dbl) data type used for numbers and precinct is a character (chr) data type used for letters/words.
```{r}
...
**Answer**: 402345. NA is used to signify missing data. Either they didn't report votes by mails as it's own separate category or they named it something else.
...
**Answer** The first 20 results suggests that voters have passed away and weren't updated. The youngest voter of the first 20 would have to be at least 84 and it would be highly unlikely that they are registered to vote at that age. 
```{r}
yadkin_voters %>% 
...
**Answer** Yes one of the things that stand out to me is moved from county and moved within state. I wonder what the difference in those data sets are. Also what is legacy data and does it prevent individuals from voting? What constitutes a confirmation returned undeliverable. 
...
**Answer** Headers are inconsistently styled, including first column starting with a number and last_name column being in all caps. The amount column is stored as a character (chr) instead of a number (dbl). Some zip codes have 6- digits and others have 9 digits. 
...
**Answer** This function adds an x to 1_linenumber to make it x1_linenumber. The function cleans headers including lowercasing column titles and correct headers that start with a number in front of it. 
...
**Answer** It is attempting to sum the variable amount when amount is being recognized as a character (chr) instead of a number (dbl). 
...
**Answer** it cleaned up the first column linenumber by getting rid of x1_ and it also got rid of the column dupe_count.
...
**Answer** All the zip codes now have 5 characters
...
**Answer** It renames x1_linumber header to linenumber. It mutates amount so that it would be digits instead of characters. It also mutates zip code to have 5 characters. And lastly, it capitalizes the first letter of the cities. 
...
**Answer** This code looks throught the city column and any Conowingo spelling that isn't spelled like correctly this: "Conowingo", gets changed to the correct spelling.
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
