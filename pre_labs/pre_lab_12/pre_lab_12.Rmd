...
**Answer** It displays table 1 of the the two listed tables. Table 1 is two different sets of voting data. 
...
**Answer** It cleans up the first set of data and removes the second set of data. It cleans the first set of data by getting rid of the first row, renaming the columns, adding a month and year column, removing commas from the data by making it a numeric, gets rid of unnecessary columns such as v12. 
...
**Task** Run the following code and describe the result.
**Answer** This one displays the second data chart and gets rid of the first one. It also cleans the chart. It renams the columns, gets rid of the commas by making it a numeric, gets rid of unnecessary columns such as v22, it removes any rows with NA. 
...
**Answer** I think the most interesting percentages are the overwhelming number of democrats that register by mail. A lot of the times, election results are not finalized because of mail-in ballots but you could call the results if majority of these ballots are for democrats. People are more likely to vote by mail if they also registered by mail. 
...
**Task** Run the following code and describe what you think are the most interesting percentages in the dataframe.
**Answer** I think the most interesting percentage is the county transfer out percentage. The percentage of registrations removed because an individual moved out of the county. A lot of people may be unaware of having their registration removed because of moving out of the county so it would nice to inform them about it. 
...
**Task** Run the following code. What's the name of the new dataframe? new_registrations_2022_06
...
### Task 12: Calculate percentages and differences
**Task** Run the following code. What stands out to you about the differences columns?
**Answer** The 16% difference in in-person registrations
...
### Task 13: Make a simple bar chart
**Task** Run the following code and describe what the chart is showing. What is the trend in the data?
**Answer** A lot of non in-person registrations such as the Eric Report, Volunteer, Mail, Online Registration. Registration is not occuring in person as much.
...
### Task 14: Produce a side-by-side chart with two plots
**Task** Run the following code to make a better bar chart. What are the main differences between them?
**Answer** In person voting has decline from June to October. While the Eric Report has shot up to the top as well volunteer. 
